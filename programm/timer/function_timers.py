"""
Функции для работы с объектами таймеров.
create_new_timer - создать новый объект таймера
get_timers_list - создает список объектов таймеров на основании словаря таймеров
prints_list_timers - Функция для вывода списка таймеров на экран
get_name_number_task - Вывод списка всех таймеров и выбор пользователя таймера который ему нужен
get_list_numbers_timers - Функция для получения списка лишь номеров всех таймеров
get_list_names_timers - Функция для получения списка лишь названий всех таймеров
get_lists_numbers_and_names_timers - Функция для получения списка номеров таймеров и названий таймеров
create_lists_hour_min_sec - Создание списка часов, минут и секунд от всех таймеров
configure_data_to_pandas - Конфигурация всех данных в фрейм pandas
view_data_in_pandas_table - Вывод таблицы (фрейма) данных всех таймеров
view_all_information_timers - Получить всю информацию в виде таблицы (фрейма) об таймерах
get_timer_by_number - Получить объект таймера по его номеру
get_list_seconds_timers - Получить список секунд, которые были насчитаны на всех таймерах
"""
...
def create_new_timer(name_timer: str) -> dict[str, Timer]:
    ...
    timer_object: Timer = Timer(name_timer)
    # Делаем помещение этого объекта таймера в общий словарь наших таймеров
    timers_dict: dict[str, Timer] = {name_timer : timer_object}
    return timers_dict

def get_timers_list(all_timers_dict: dict[str, Timer]) -> list[Timer]:
    """
    ...
    """
    timers_object_list_values: dict_values[Timer] = all_timers_dict.values()
    timers_object_list: list[Timer, Timer] = list(timers_object_list_values)
...
def prints_list_timers(timers_object_list: list[Timer, Timer]) -> None:
    """
...
def get_name_number_task(timers_object_list: list[Timer], task_message: str):
    """
...
def get_list_numbers_timers(timers_object_list: list[Timer]) -> list[int, int]:
    """
...
def get_timer_by_number(timers_object_list: list[Timer], number: int) -> Timer:
    """
    ...
    timer_object: Timer = timers_object_list[index_object_timer]
    return timer_object

def get_list_seconds_timers(timers_object_list: list[Timer]) -> list[int, int]:
    """
